...
Ranking queries are notorious for being computationally intensive - aka slow. Some solutions will use statistics to speed up a ranking query by approximating the true results, but that’s not always a desirable option. In addition to SQL, FeatureBase has a native language called [PQL](/docs/pql-guide/pql-home). In PQL, [```TopK```](/docs/pql-guide/read-topk) queries can be run to return exact results in milliseconds. 
