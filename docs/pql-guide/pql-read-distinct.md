...
`Distinct()` can always be used as a [row call](/docs/pql-guide/pql-read-home#row-calls). This, in conjunction with the index argument, gives users the ability to query across indexes. Use caution to ensure you get the expected behavior. The examples below outline some considerations.
